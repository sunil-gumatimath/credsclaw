...
GITHUB_TOKEN_PATTERN = r"\b(?:ghp_[0-9a-zA-Z]{36,40}|gho_[0-9a-zA-Z]{36}|ghs_[0-9a-zA-Z]{36,200}|ghr_[0-9a-zA-Z]{36}|ghu_[0-9a-zA-Z]{36}|github_pat_[0-9a-zA-Z]{22}_[0-9a-zA-Z]{59})\b"
...
PLURAL_LIST_KEYS = {"providers", "allow_patterns", "deny_patterns", "extensions"}
...
def apply_config_to_parser(config: dict, parser: argparse.ArgumentParser) -> None:
  ...
  - repo: local
    hooks:
      - id: api-key-auditor
        ...
        entry: python auditor.py --mode local --dir . --confidence-threshold 60.0 --dry-run
        language: system
        types: [text]
...
class APIAuditor:
    ...
    async def audit_git_history(self, provider: str, pattern: str, directory: str) -> None:
        """Scan local git commit history for exposed API keys.

        Uses ``git log --all -p`` to inspect every commit diff
        across all branches.
        """
        ...
        if self.args.dry_run:
            logger.info("[Dry run] git history scan for %s in %s", provider, directory)
            return
        ...
        async def process_commit(commit: Dict[str, str]) -> None:
            ...
            # Rate-limit concurrent subprocess calls
            async with self.semaphore:
                # Get the full diff for this commit
                try:
                    # ``git show`` diff without commit message
        ...
        iterator = asyncio.as_completed(tasks)
        if tqdm:
            iterator = tqdm(iterator, total=len(tasks), desc=f"Git history {provider}")
        for coro in iterator:
            await coro
        ...
        self.progress.save_progress()
        logger.info("Completed %s git-history audit: %s total unique keys found", provider, len(self.progress.found_keys))
...
<body>
<h1>\U0001f6e1\ufe0f API Key Auditor Report</h1>
...
Examples:
  ...
  Local directory scan:
    python auditor.py --mode local --dir ./project --providers openai,aws

  Git history scan:
    python auditor.py --mode git-history --dir ./repo --providers github,slack

  Use config file:
    python auditor.py --config auditor.yaml
...
""",
    ...
    parser.add_argument("--dir", type=str, help="Local directory path to scan (for mode=local or git-history)")
    parser.add_argument("--mode", type=str, choices=["code", "commits", "local", "git-history"], default="code",
                        help="Search mode: code, commits, local, or git-history")
    parser.add_argument("--validate", action="store_true", help="Validate found API keys against provider APIs")
    parser.add_argument("--output-format", type=str, choices=["json", "csv", "txt", "html"], default="json",
                        help="Output format (json, csv, txt, html)")
    parser.add_argument("--output-file", type=str, default="", help="Output file path")
    ...
    parser.add_argument("--timeout", type=int, default=DEFAULT_VALIDATION_TIMEOUT, help="Validation request timeout in seconds")
    ...
    # Utility
    parser.add_argument("--generate-pre-commit-hook", action="store_true",
                        help="Generate a .pre-commit-config.yaml file for pre-commit integration")
...
def parse_args(argv: Optional[List[str]] = None, config: Optional[dict] = None) -> argparse.Namespace:
    ...
    if not args.output_file:
        ...
        args.output_file = f"audit_results.{ext}"
...
async def main() -> None:
    ...
    early_parser.add_argument("--config", type=str, default=DEFAULT_CONFIG_FILE)
    ...
    try:
        ...
        async with APIAuditor(token, rate_limiter, progress, args) as auditor:
            # Build coroutines for all selected providers (parallel execution)
            provider_tasks = []
            for provider_key in selected:
                ...
                if args.mode == "local":
                    ...
                    provider_tasks.append(auditor.audit_local_directory(name, pattern, args.dir))
